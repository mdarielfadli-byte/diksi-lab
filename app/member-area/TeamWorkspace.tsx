...
export function TeamWorkspace({ userEmail, initialCompanyId, isSuperAdmin, onSignOut, onViewDashboard }: { userEmail: string; initialCompanyId: string; isSuperAdmin: boolean; onSignOut: () => Promise<void>; onViewDashboard: (company: Company, projectId: string | null) => void }) {
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    const companyResponse = isSuperAdmin ? await supabase.from("companies").select("id, name, slug").order("name") : ids.length ? await supabase.from("companies").select("id, name, slug").in("id", ids).order("name") : { data: [], error: null };
  ...
  async function updateTask(event: FormEvent<HTMLFormElement>) {
    ...
    await withAction(async () => { const supabase = getSupabaseBrowserClient(); const { data: auth } = await supabase.auth.getUser(); if (!auth.user) throw new Error("Sesi masuk berakhir.");
      const status = String(form.get("status")); const { error: updateError } = await supabase.from("project_tasks").update({ status, progress: Number(form.get("progress")), client_visible: form.get("client_visible") === "on" }).eq("id", selectedTask.id); if (updateError) throw updateError;
      const file = form.get("attachment");
      if (file instanceof File && file.size > 0) { if (file.size > 25 * 1024 * 1024) throw new Error("Ukuran file maksimal 25 MB."); const extension = file.name.includes(".") ? file.name.split(".").pop() : "file"; const storagePath = `${companyId}/${crypto.randomUUID()}.${extension}`;
        const { error: uploadError } = await supabase.storage.from("client-documents").upload(storagePath, file, { contentType: file.type || undefined, upsert: false }); if (uploadError) throw uploadError;
        const { error: fileError } = await supabase.from("task_files").insert({ task_id: selectedTask.id, company_id: companyId, project_id: projectId, uploaded_by: auth.user.id, name: file.name, storage_path: storagePath, content_type: file.type || null, byte_size: file.size, review_status: status === "needs_review" ? "needs_review" : "draft", client_visible: form.get("file_client_visible") === "on" }); if (fileError) throw fileError;
      }
      if (updateFileInput.current) updateFileInput.current.value = ""; setMessage(file instanceof File && file.size > 0 ? "Task dan file output diperbarui. Progress proyek mengikuti status task." : "Status task diperbarui."); });
  }
